...
app.controller("ModuleController", function ($scope, $http, $timeout) {
    ...
    $scope.findAndShowCase = function () {
        ...
        var caseId = $scope.caseNumber;
    
        if (caseId) {
            
            httpGet('/api/module/lt_case/get/' + caseId, function (res) {
            
                $scope.shownCase = res;
                $scope.caseNumber = null;  
            }, function () {
             
                httpGet('/api/module/lt_policy/get/' + caseId, function (res) {
                   
                    $scope.shownCase = res;
                    $scope.caseNumber = null;
                }, function () {
                
                    console.error('Case or policy not found for id: ' + caseId);
                });
            });
        } else {
          
            console.error('Please provide a valid case number.');
        }
    };
