...
{   
    ...
    function addCustomerToCase(Request $request, $id) {
        ...
        if ($validator->fails()) {
            ...
            $customer = Customer::firstOrCreate(['id_number' => $validated['id_number']]);
    
        
            if (!$case->customers()->where('customer_id', $customer->id)->exists()) {
             
                $case->customers()->attach($customer->id);
            }
        }
...
SQL);
    ...
    function getCustomersWithPolicyNames(Request $request) {
        $results = DB::table('lt_customer')
            ->leftJoin('policy_customer', 'lt_customer.id', '=', 'policy_customer.customer_id')
            ->leftJoin('policy', 'policy_customer.policy_id', '=', 'policy.id')
            ->select(
                'lt_customer.last_name',
                'lt_customer.first_name',
                'lt_customer.id_number',
                DB::raw('GROUP_CONCAT(policy.policy_name) as policy_names')
            )
            ->groupBy('lt_customer.id')
            ->get();
